...
export const Autocomplete = memo((_a) => {
    var { value, disabled, label, solo, delay = 300, items, solid, onChange, onSearch, renderItem, color = "primary", autoFiltering = true, flatListProps, clearText, clearable, children } = _a, props = __rest(_a, ["value", "disabled", "label", "solo", "delay", "items", "solid", "onChange", "onSearch", "renderItem", "color", "autoFiltering", "flatListProps", "clearText", "clearable", "children"]);
    ...
    const onSelect = useCallback((newValue) => onChange === null || onChange === void 0 ? void 0 : onChange(newValue), [onChange]);
    ...
    const filteredItems = useMemo(() => {
        if (autoFiltering)
            return items === null || items === void 0 ? void 0 : items.filter(({ label: label2 }) => label2 !== currentText &&
                label2
                    .toLowerCase()
                    .normalize("NFD")
    ...
    const separator = useCallback(() => _jsx(Divider, {}), []);
    const content = (_jsx(FlatList, Object.assign({ data: currentItems, removeClippedSubviews: true, renderItem: renderOption, ItemSeparatorComponent: separator, style: styles.list }, flatListProps)));
    return (_jsxs(View, Object.assign({ ref: ref, style: styles.container }, { children: [children || (_jsx(TextField, Object.assign({}, props, { label: label, disabled: disabled, color: color, value: currentText, onFocus: openPopover, onBlur: onBlur, containerStyle: solid && !!filteredItems.length && styles.noBorderInput, onChange: onChangeText }))), solid && !!filteredItems.length && (_jsx(Card, Object.assign({ variant: "outlined", style: styles.card }, { children: content }))), !solid && (_jsx(Popover, Object.assign({ solo: true, parentRef: ref, open: isFocused && !!filteredItems.length }, { children: content })))] })));
});
