...
export const SelectField = memo((_a) => {
    var { value, disabled, label, items, multiple, searchable, onChange, renderItem, color = "primary", flatListProps, clearText, clearable, children } = _a, props = __rest(_a, ["value", "disabled", "label", "items", "multiple", "searchable", "onChange", "renderItem", "color", "flatListProps", "clearText", "clearable", "children"]);
    const theme = useTheme();
    ...
    const renderOption = useCallback(({ item, index }) => {
        ...
        return (_jsx(SelectFieldItem, Object.assign({ selected: isSelected, color: color, nativeID: item.nativeID, disabled: item.disabled, value: item.value, onPress: onChangeValue }, { children: (renderItem === null || renderItem === void 0 ? void 0 : renderItem(item, isSelected)) || item.label }), index));
