...
export const SelectField = memo((_a) => {
    ...
    const renderOption = useCallback(({ item, index }) => {
        ...
        return (_jsx(SelectFieldItem, Object.assign({ selected: isSelected, color: color, nativeID: item.nativeID, disabled: item.disabled, value: item.value, onPress: onChangeValue }, { children: (renderItem === null || renderItem === void 0 ? void 0 : renderItem(item, isSelected)) ||
                (multiple ? (_jsx(Checkbox, { disabled: item.disabled, value: isSelected, label: item.label, pointerEvents: "none" })) : (item.label)) }), index));
