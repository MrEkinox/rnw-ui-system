...
export const Card = memo((_a) => {
    var { children, square, variant = "blank", elevation = 5, color, style } = _a, props = __rest(_a, ["children", "square", "variant", "elevation", "color", "style"]);
    const theme = useTheme();
    const borderRadius = square ? 0 : theme.borderRadius;
    const themeColor = color
        ? theme.palette[color] || color
        : theme.palette.primary;
    const backgroundColor = color && variant === "blank" ? themeColor : theme.palette.background.card;
