...
export const TextFieldHelper = memo(({ error, color, children }) => {
    ...
    if (typeof children !== "string")
        return null;
    return (_jsx(Typography, { variant: "caption", color: error ? "error" : color, style: style, children: children }));
});
...
export const TextFieldLabel = memo(({ required, isActive, height, disabled, error, color = "primary", children, }) => {
    ...
    if (typeof children !== "string")
        return null;
    return (_jsx(Animated.Text, { style: style, numberOfLines: 1, children: required ? `${children}*` : children }));
});
TextFieldLabel.displayName = "TextFieldLabel";
export const TextField = memo(({ value = "", disabled, onChange, label, editable = true, startIcon, endIcon, required, error, onBlur, onFocus, helperText, containerStyle, autoGrow, color = "primary", name, keyboardType, ...props }) => {
    const theme = useTheme();
    ...
    return (_jsxs(_Fragment, { children: [_jsxs(View, { style: style, children: [memoStartIcon, _jsxs(View, { style: flexStyle, children: [_jsx(TextFieldLabel, { required: required, isActive: labelIsActive, disabled: disabled, color: color, error: error, children: label }), _jsx(TextInput, { ref: inputRef, accessibilityLabel: label, nativeID: name, ...props, keyboardType: keyboardType, onChangeText: onChangeValue, onBlur: onMemoBlur, onFocus: onMemoFocus, style: inputStyle, onContentSizeChange: onContentSizeChange, placeholderTextColor: placeholderTextColor, value: value, editable: canEdit, selectionColor: themeColor })] }), memoEndIcon] }), _jsx(TextFieldHelper, { color: color, error: error, children: helperText })] }));
});
