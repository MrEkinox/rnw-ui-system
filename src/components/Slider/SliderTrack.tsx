...
export const SliderTrack = memo<SliderTrackProps>(
  ({ color, values, maxValue, minValue, inInteraction, step }) => {
    const currentMinValue =
      values.length === 1 ? minValue : Math.min(...values);
    ...
    const percentMin =
      ((minCurrentValue - minValue) * 100) / (maxValue - minValue);
    const percentMax =
      ((maxCurrentValue - minValue) * 100) / (maxValue - minValue) - percentMin;
    const animateMaxPosition = useMemo(
      () => new Animated.Value(percentMax),
      [percentMax]
    );

    const animateMinPosition = useMemo(
      () => new Animated.Value(percentMin),
      [percentMin]
    );
